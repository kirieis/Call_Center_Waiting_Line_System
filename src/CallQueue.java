...
/**
 * Lớp hàng đợi cuộc gọi (CallQueue) sử dụng PriorityQueue để tự động sắp xếp cuộc gọi
 * dựa trên mức độ ưu tiên (VIP, số lần gọi lại) và thứ tự thời gian gọi vào (FIFO).
 */
public class CallQueue {
     ...
     * 2. Nếu điểm ưu tiên bằng nhau, cuộc gọi nào gọi trước (orderNumber nhỏ hơn) sẽ được phục vụ trước (FIFO).
     */
    public CallQueue() {
        waitingCalls = new PriorityQueue<>(new Comparator<Call>() {
            ...
            public int compare(Call first, Call second) {
                ...
                // Nếu điểm ưu tiên bằng nhau, sắp xếp theo số thứ tự tăng dần (ai vào trước phục vụ trước)
    ...
    /**
     * Thêm một cuộc gọi mới vào hàng đợi. PriorityQueue sẽ tự động chèn vào đúng vị trí ưu tiên.
     */
    ...
    public void showWaitingCalls() {
        ...
        // Tạo một danh sách tạm thời từ PriorityQueue để sắp xếp và hiển thị mà không ảnh hưởng tới hàng đợi gốc
